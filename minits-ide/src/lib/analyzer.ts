...
export function analyze(ast: Program): CompilationError[] {
    const errors: CompilationError[] = [];
    const symbolMap = new Map<string, { type: string, line: number, column: number }>();
    ...
    ast.vars.forEach((v) => {
        if (symbolMap.has(v.identifier)) {
            ...
            symbolMap.set(v.identifier, { type: v.varType, line: v.line, column: v.column });
        }
    });
        ...
        switch (stmt.type) {
            case 'Assignment':
                if (!symbolMap.has(stmt.identifier)) {
                    ...
                    const expectedType = symbolMap.get(stmt.identifier)?.type;
            ...
            case 'ForStatement':
                if (stmt.init.type === 'VarDecl') {
                    ...
                    symbolMap.set(stmt.init.identifier, { type: stmt.init.varType, line: stmt.init.line, column: stmt.init.column });
                } else {
    ...
    function checkExpression(expr: Expression): string | null {
        switch (expr.type) {
            ...
            case 'Identifier':
                ...
                return symbolMap.get(expr.name)!.type;
            case 'BinaryExpr':
                ...
                if (['+', '-', '*', '/'].includes(expr.operator)) {
                    // String concatenation is somewhat allowed with +, but let's strictly stick to numbers for math if MiniTS specifies it
                    if (expr.operator === '+' && (leftType === 'string' || rightType === 'string')) {
                        return 'string';
                    }
                    if (leftType !== 'number' || rightType !== 'number') {
                        if (leftType && rightType) {
                            reportSemanticError(`Operador aritmético '${expr.operator}' no soportado entre '${leftType}' y '${rightType}'`, expr.line, expr.column);
                        }
                    }
                    return 'number';
                }
